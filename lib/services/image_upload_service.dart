...
class ImageUploadService {
  ...
  static Future<String> uploadImage(
    ...
    try {
      ...
      // Pré-processar a imagem para formato quadrado 1:1 antes do upload
      try {
        file = await processImageToSquare(file);
      } catch (e) {
        // Em caso de falha no processamento, loga e continua com a imagem original
        // ignore: avoid_print
        print('Falha ao pré-processar imagem antes do upload: $e');
      }
  ...
  /// Faz upload de uma imagem específica para banner de produto.
  ///
  /// Usa processamento horizontal (16:9) antes do upload para melhor encaixe no banner.
  static Future<String> uploadBannerImage(
    ...
    try {
      ...
      // Pré-processar a imagem para formato horizontal 16:9 antes do upload
      try {
        file = await processImageToBanner(file);
      } catch (e) {
        // ignore: avoid_print
        print('Falha ao pré-processar imagem de banner antes do upload: $e');
      }
      ...
      final timestamp = DateTime.now().millisecondsSinceEpoch;
      final extension = localPath.split('.').last;
      final fileName = '$userId/banner_${timestamp}.$extension';
      ...
      throw Exception('Erro ao fazer upload da imagem de banner: $e');
    }
  }
  ...
  /// Upload de imagem de banner de produto
  static Future<String> uploadProductBannerImage(String localPath) async {
    return uploadBannerImage(localPath, folder: 'product-images');
  }
