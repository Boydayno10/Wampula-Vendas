...
class _PedidoDetalheScreenState extends State<PedidoDetalheScreen> {
  ...
  Future<void> _falarComVendedor() async {
    final orderId = _currentOrder.id;

    try {
      setState(() => _isLoading = true);

      // Buscar o vendedor associado a este pedido
      final sellerId = await SellerProductService.getSellerIdByOrderId(orderId);
      ...
      if (sellerId == null) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
              content: Text(
                'Não foi possível localizar o vendedor deste pedido.',
              ),
      ...
      // Montar título do chat no formato "Loja - Pedido"
      String headerTitle = 'Chat';
      try {
        if (_currentOrder.items.isNotEmpty) {
          final firstItem = _currentOrder.items.first;

          // Buscar produto do vendedor para obter o nome da loja
          final sellerProduct = await SellerProductService.getProductById(
            firstItem.id,
          );

          final storeName = sellerProduct?.sellerStoreName ?? 'Loja';

          // Abreviar nome da loja se for muito comprido para
          // deixar espaço para o nome do produto
          String shortStoreName = storeName;
          const maxStoreLength = 20;
          if (shortStoreName.length > maxStoreLength) {
            shortStoreName = shortStoreName.substring(0, maxStoreLength) + '…';
          }

          // Exibir: Nome da Loja - ID do Pedido (ex: Justino Store - WP-00001)
          headerTitle = '$shortStoreName - $orderId';
        }
      } catch (_) {
        // Em caso de qualquer erro, mantém um título genérico
        headerTitle = 'Chat';
      }

      // Criar ou recuperar chat direto entre cliente e vendedor,
      // associando este chat ao código do pedido (orderId)
      ...
      Navigator.push(
        ...
        MaterialPageRoute(
          builder: (_) =>
              ChatScreen(chatId: chat.id, publicationName: headerTitle),
        ),
      );
    } catch (e) {
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text('Erro ao abrir chat com o vendedor: $e'),
            backgroundColor: Colors.red,
          ),
        );
      }
  ...
  Widget buildActionButtons() {
    ...
    // Priorizar o fluxo de reembolso pelo RefundStatus
      ...
      case RefundStatus.approved:
        return Column(
          children: const [
            ...
            SizedBox(height: 8),
            Text(
              'O seu reembolso será processado',
              style: TextStyle(
                color: Colors.green,
                fontSize: 16,
                fontWeight: FontWeight.bold,
            ...
            SizedBox(height: 4),
            Text(
              'O valor será devolvido em breve.',
      ...
      case RefundStatus.denied:
        return Column(
          ...
          children: [
            ...
            fullButton('Falar com o vendedor', _falarComVendedor),
          ],
        );
    ...
    // Fluxo padrão quando não há reembolso em andamento
    switch (order.status) {
      ...
      case OrderStatus.reembolsoSolicitado:
        // Fallback para casos legados sem RefundStatus preenchido
        return Column(
          children: [
            Container(
              padding: const EdgeInsets.all(12),
                ...
                Container(
                  ...
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(height: 4),
                      Text(
                        'Estado: ${statusLabel(order.status, order.refundStatus)}',
                      ),
                      const SizedBox(height: 12),
                      Row(
