...
class _SellerOrdersScreenState extends State<SellerOrdersScreen> {
  ...
  Future<void> _updateOrderStatus(
    ...
    try {
      print(
        '🔄 Atualizando status do pedido ${order.id} para ${newStatus.name}',
      );
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(
              'Status atualizado para: ${_getStatusName(newStatus)}',
            ),
  ...
  String _getStatusName(SellerOrderStatus status) {
    switch (status) {
      ...
      case SellerOrderStatus.reembolsoSolicitado:
        return 'Reembolso solicitado do pedido';
    }
  }
  ...
  Future<void> _showDenyRefundDialog(SellerOrderModel order) async {
          ...
          children: [
            ...
            TextField(
              ...
              decoration: const InputDecoration(
                hintText:
                    'Ex: Produto já foi entregue e está em perfeitas condições',
    ...
    try {
      final chatId = await SellerProductService.denyRefund(order.id, reason);
      ...
      if (mounted) {
        ...
        // Se um chat foi criado/recuperado, abrir conversa com o cliente
        if (chatId != null && chatId.isNotEmpty) {
          try {
            final chat = await ChatService.getChatById(chatId);
            if (chat != null) {
              // Para o vendedor, exibir apenas o ID do pedido
              // (ex: WP-00001) como título do chat
              final String header = order.id;
              ...
              Navigator.push(
                ...
                MaterialPageRoute(
                  builder: (_) =>
                      ChatScreen(chatId: chat.id, publicationName: header),
                ),
              );
            }
          } catch (e) {
            // Não bloquear fluxo se der erro ao abrir o chat
            debugPrint('Erro ao abrir chat após negar reembolso: $e');
          }
        }
      }
    } catch (e) {
        ...
        builder: (context, scrollController) => ListView(
          ...
          children: [
                        ...
                        Wrap(
                          ...
                          children: [
                            ...
                            if (order.storage != null &&
                                order.storage!.isNotEmpty)
                              _buildOptionChip(
                                'Armazenamento: ${order.storage}',
                              ),
                            if (order.age != null && order.age!.isNotEmpty)
                              _buildOptionChip('Idade: ${order.age}'),
                            if (order.pantSize != null &&
                                order.pantSize!.isNotEmpty)
                              _buildOptionChip('Calça: ${order.pantSize}'),
                            if (order.shoeSize != null &&
                                order.shoeSize!.isNotEmpty)
            ...
            if ((order.status == SellerOrderStatus.reembolsoSolicitado ||
                    order.status == SellerOrderStatus.cancelado) &&
            ...
            _buildInfoRow(
              Icons.local_offer,
              'Comissão (10%)',
              '${(order.total * 0.1).toStringAsFixed(2)} MT',
              color: Colors.red,
            ),
            _buildInfoRow(
              Icons.account_balance_wallet,
              'Você Recebe',
              '${(order.total * 0.9).toStringAsFixed(2)} MT',
              color: Colors.green,
              bold: true,
            ),
            ...
            if (order.processedAt != null)
              _buildInfoRow(
                Icons.check,
                'Processado em',
                DateFormat('dd/MM/yyyy HH:mm').format(order.processedAt!),
              ),
            ...
            // Ações Normais
            if (order.status != SellerOrderStatus.entregue &&
                order.status != SellerOrderStatus.cancelado &&
                order.status != SellerOrderStatus.reembolsoSolicitado) ...[
              const Text(
                'Alterar Status',
                style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16),
              ),
              ...
              // Quando o cliente solicitou reembolso, o cancelamento
              // deve ser feito pelos botões específicos de
              // "Aprovar Reembolso" / "Negar Reembolso".
              // Portanto, escondemos o botão genérico de cancelar aqui
              // para garantir que o fluxo de recusa use SellerProductService.denyRefund.
              if (order.status != SellerOrderStatus.reembolsoSolicitado)
                OutlinedButton.icon(
                  onPressed: () {
                    Navigator.pop(context);
  ...
  // Nega o reembolso com motivo padrão e abre o chat com o cliente
  Future<void> _handleDenyRefund(SellerOrderModel order) async {
    const String defaultReason = 'Reembolso negado pelo vendedor';

    try {
      final chatId = await SellerProductService.denyRefund(
        order.id,
        defaultReason,
      );
      ...
      if (mounted) {
        ...
        if (chatId != null && chatId.isNotEmpty) {
          try {
            final chat = await ChatService.getChatById(chatId);
            if (chat != null) {
              // Sempre exibir o ID do pedido no topo do chat
              String header = 'Pedido #${order.id}';
              if (order.productName.isNotEmpty) {
                header = 'Pedido #${order.id} • ${order.productName}';
              }
        ...
        actions: [
          PopupMenuButton<SellerOrderStatus?>(
            ...
            itemBuilder: (_) => [
              ...
              const PopupMenuItem(
                value: SellerOrderStatus.novo,
                child: Text('Novos'),
              ),
              const PopupMenuItem(
                value: SellerOrderStatus.processando,
                child: Text('Processando'),
              ),
              const PopupMenuItem(
                value: SellerOrderStatus.enviado,
                child: Text('Enviados'),
              ),
              const PopupMenuItem(
                value: SellerOrderStatus.entregue,
                child: Text('Entregues'),
              ),
              const PopupMenuItem(
                value: SellerOrderStatus.reembolsoSolicitado,
                child: Text('Reembolso solicitado'),
              ),
              const PopupMenuItem(
                value: SellerOrderStatus.cancelado,
                                        ...
                                        children: [
                                          ...
                                          if (_hasOptions(order))
                                            Wrap(
                                              ...
                                              children: [
                                                if (order.size != null &&
                                                    order.size!.isNotEmpty)
                                                  _buildOptionChip(
                                                    'Tamanho: ${order.size!}',
                                                  ),
                                                if (order.color != null &&
                                                    order.color!.isNotEmpty)
                                                  _buildOptionChip(
                                                    'Cor: ${order.color!}',
                                                  ),
                                                if (order.storage != null &&
                                                ...
                                                if (order.pantSize != null &&
                                                    order.pantSize!.isNotEmpty)
                                                  _buildOptionChip(
                                                    'Calça: ${order.pantSize!}',
                                                  ),
                                                if (order.shoeSize != null &&
                                                    order.shoeSize!.isNotEmpty)
                                                  _buildOptionChip(
                                                    'Calçado: ${order.shoeSize!}',
                                                  ),
                                              ],
                                            ),
                                          const SizedBox(height: 6),
