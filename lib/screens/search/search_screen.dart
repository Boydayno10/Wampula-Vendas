...
class _SearchScreenState extends State<SearchScreen> {
  ...
  Future<void> _loadPopularSearches() async {
    try {
      // 1️⃣ Tenta carregar termos PERSONALIZADOS do usuário
      final userRows = await ProductAnalyticsService.getUserPopularSearchTerms(
        limit: 9,
      );
      List<String> terms = userRows
      ...
      if (terms.isNotEmpty) {
        final seen = <String>{};
        terms = terms.where((t) => seen.add(t)).toList();
      }

      // 2️⃣ Se o usuário ainda não tem histórico, cai para os termos GLOBAIS
      if (terms.isEmpty) {
        final globalRows = await ProductAnalyticsService.getPopularSearchTerms(
          limit: 9,
        );
        terms = globalRows
            .map((e) => (e['search_term'] as String?)?.trim())
          ...
          if (scrollInfo.metrics.extentAfter < 200 && total > clampedVisible) {
            ...
            Future.delayed(_loadMoreDelay, () {
              ...
              setState(() {
                ...
                _visibleResultsCount = next > total ? total : next;
                _isLoadingMoreResults = false;
              });
            });
          }
